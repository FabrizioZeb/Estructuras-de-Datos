...
//Hemos modificado la función add para que no ordene el set a la hora de sacar por pantalla los resultados
//correspondientes.
void SetOfInts3::add(int x){ //throws Error
	bool found;
	int pos;

	Linealsearch(x,found);
	if (!found){
		if (isFull())
...
void SetOfInts3::Linealsearch(int x, bool & found) const {
    int i = 0;
    while(i < size && !found){
        if(elems[i] == x) found = true;
        i++;
    }
}
