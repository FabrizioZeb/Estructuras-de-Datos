...
public:
	...
	//Coste O(n)
	void colar(const T & elem, Queue<T> colegas){
        Nodo* i = this->_prim;
        Nodo* enl = this->_prim;
        bool enc = false;
        //Buscar al pringao
        while (!enc && i->_sig != NULL){
            if(i->_elem == elem){
                enc = true;
                enl = i->_sig;
            }
        ...
        //Si lo encuentra modificar los nodos poniendose delante del pringao los colegas
        if(enc){
            Nodo *act = colegas._prim;
            //Enlazamos al pringao con el primer colega.
            i->_sig = act;
            colegas._ult->_sig = enl;
            //Enlazar el último colega detrás del resto de la cola ("sus amigos")

            //Modificar el num de elementos de la cola.
            _numElems = colegas.size() + this->_numElems;
        }
